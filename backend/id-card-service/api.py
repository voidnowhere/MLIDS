...
def extract():
    ...
    results = text_recognition(temp_path, east_path)
    ...
    for ((startX, startY, endX, endY), text) in results:
        text = text.strip()
        if re.match('[A-Z][A-Z]?[0-9]{6}', text):
            data['cin'] = text
        if re.match('[A-Z]{3,}', text):
            data['last_name'] = text
            if isSeen == True:
...
def text_recognition(image_path, east_path, min_confidence=0.5, width=320, height=320, padding=0.0, language='eng',
                     oem=1, psm=7):
